...
The March 6–7, 2026 GardaWorld + KVG award pair is the **emblematic example** of WEXMAC-TITUS's operating-contract pattern: billion-dollar-ceiling contracts awarded to operators with no detention experience, using a DoD-administered IDIQ rather than a fresh ICE solicitation. **Note the limit of that characterization for this specific award**: KVG's task order was competed. Its own FPDS record reads `extent_competed` = "A" (FULL AND OPEN COMPETITION), `solicitation_procedures` = "MAFO" (SUBJECT TO MULTIPLE AWARD FAIR OPPORTUNITY), **4 offers received**, `fair_opportunity_limited_sources` = null, `other_than_full_and_open` = null. "Bypasses normal ICE contracting oversight" is accurate only in the narrow sense that ICE rode a Navy-administered vehicle (parent IDV **N0002325D0048**) instead of running its own solicitation — not in the sense of no competition. [^kvg-competition-note] Both contractors are novel to the detention-industrial complex — GardaWorld as the Aegis Defence mercenary-lineage operator entering U.S. domestic detention, and KVG as a logistics contractor scaling ~50× above its prior single-contract maximum.
...
[^kvg-competition-note]: **Sourcing note (2026-08-28 pattern-projection audit):** verified against the award's own contracting record, USAspending award 355838775, PIID `70CDCR26FR0000035`, recipient UEI HERXJEEWZFL7, total obligation $113,052,115.78 (the "$113 million" figure checks out precisely). Competition fields: `extent_competed` = A (FULL AND OPEN COMPETITION); `solicitation_procedures` = MAFO; `number_of_offers_received` = 4; `fair_opportunity_limited_sources` = null; `other_than_full_and_open` = null. Parent IDV `N0002325D0048` (DoD, INDEFINITE DELIVERY / INDEFINITE QUANTITY, MULTIPLE AWARD) — note this is a *different* WEXMAC seat than the GardaWorld Surprise award's parent (`N0002325D0032`). This is the same failure shape corrected on the GardaWorld entry the same day: the WEXMAC-TITUS warehouse-acquisition-bypass pattern is real for the property purchases, but it does not transfer to these two operating task orders, both of which were competed under fair-opportunity procedures. The genuinely non-competed awards in this thread (Bi2 iris scanners, Amentum Camp East Montana, SNA rapid DNA, Penlink, CoreCivic Torrance/Cibola, GEO Big Horn, Our Rescue) all carry `extent_competed` = B or C and 1 offer — none of them ran through a WEXMAC task order.
